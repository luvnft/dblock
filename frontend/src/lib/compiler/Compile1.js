...
const Compile1 = ({ isNewUser }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                setHints(data[1].Hints)
                setAddId(data[1]._id)
                setFetchedData(data[1]);
                if (data.length > 0) {
                    setBoilerPlate(data[1].BoilerPlate); // Assuming the first problem's boilerplate
                    setCodeInput(data[1].BoilerPlate);
                    setByteCode(data[1].ByteCode)
                    setUrl('/compiler2')
                }
            } catch (error) {
                console.log(error);
    ...
    const handleSeeHint = () => {
        ...
        setModelCode(fetchedData[1]?.BoilerPlate || 'No code available');
        setShowModal(true);
    };
