...
const Compile2 = ({ isNewUser }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                setHints(data[2].Hints)
                setAddId(data[2]._id)
                setFetchedData(data[2]);
                if (data.length > 0) {
                    setBoilerPlate(data[2].BoilerPlate); // Assuming the first problem's boilerplate
                    setCodeInput(data[2].BoilerPlate);
                    setByteCode(data[2].ByteCode)
                    setUrl('/alert')
                }
            } catch (error) {
                console.log(error);
    ...
    const handleSeeHint = () => {
        ...
        setModelCode(fetchedData[2]?.BoilerPlate || 'No code available');
        setShowModal(true);
    };
