...
const Compiler = ({ isNewUser }) => {
    ...
    const handleSubmit = async () => {
        if (selectedLanguage === "solidity") {
            try {
                ...
                // const userData = await axios.get(https://dcb-team-alpha.vercel.app/getUserId/${id})
                // const findID = userData.data.fetchedData._id;
                // const solvedId = userData.data.fetchedData.solved
                //  await axios.patch(https://dcb-team-alpha.vercel.app/updateUserPoint/${id},{
                //     points: 10,
                // })
                // await axios.patch(https://dcb-team-alpha.vercel.app/updateUser/${id},{
                //     solved:[id]
                // })
    ...
    const handleSeeHint = () => {
        setModalTitle('Hint');
        setModalMessage(fetchedData[0]?.Hints);
